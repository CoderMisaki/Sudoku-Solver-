...
## 2026-05-21 - Replace deprecated execCommand('copy')
**Vulnerability:** Deprecated `document.execCommand('copy')` poses security risks as it does not enforce secure contexts or properly manage clipboard permissions.
**Learning:** The modern asynchronous `navigator.clipboard.writeText()` API should be used for interacting with the clipboard in web applications to ensure secure and user-permissioned access.
**Prevention:** Always use the Clipboard API instead of `execCommand` for clipboard operations in new code or refactoring.
